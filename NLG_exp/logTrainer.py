...
class LogTrainerFedProx(LogTrainer):
    def __init__(self, global_state, prox_mu, **kwargs):
        super(LogTrainerFedProx, self).__init__(**kwargs)
        self.global_state = global_state
        self.mu = prox_mu

    def compute_loss(self, model, inputs, return_outputs=False):

        return_values = super(LogTrainerFedProx, self).compute_loss(model, inputs, return_outputs=return_outputs)
...
class LogTrainerSCAFFOLD(LogTrainer):
    ...
    def get_auxiliary_param(self):
        auxiliary_new_para = deepcopy(self.local_auxiliary)
        auxiliary_delta_para = deepcopy(self.local_auxiliary)
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if not param.requires_grad:
                    ...
                    auxiliary_new_para[name] = (self.global_state[name].cpu() - param.cpu()) / (
                                self.args.max_steps * self.args.learning_rate) - self.correction[name].cpu()
                    auxiliary_delta_para[name] = auxiliary_new_para[name].cpu() - self.local_auxiliary[name].cpu()
